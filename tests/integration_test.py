...
@pytest.mark.parametrize("n_clusters,max_iter", [(1, 100), (2, 100), (4, 200), (8, 200)])
def test_kmeans(synthetic_dataset: SyntheticDataset, n_clusters: int, max_iter: int):
    ...
    kmeans = KMeans(n_clusters=n_clusters, max_iter=max_iter, tol=1e-4, use_tqdm=False, n_init=100)
    km_results = kmeans.fit_transform(X)
    assignments, cluster_centers = km_results.assignments, km_results.centers

    # basic assertions
    assert cluster_centers.shape[0] == n_clusters
    ...
    assert assignments.max() == n_clusters - 1
    ...
    sk_kmeans = skKMeans(
        init="random", n_clusters=n_clusters, max_iter=max_iter, tol=1e-4, random_state=1001
    )
